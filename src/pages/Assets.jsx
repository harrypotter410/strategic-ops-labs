...
function AssetModal({ asset, onClose, onSave }) {
  const [form, setForm] = useState(asset ? {
    ...asset,
    rooms: asset.rooms || '', year_acquired: asset.year_acquired || '',
    ...
    name:'', market:'', type:'hotel', status:'active', rooms:'', brand:'',
    year_acquired:'', acquisition_price:'', current_value:'', cap_rate:'', notes:'', address:'',
  })
  const [loading, setLoading] = useState(false)
  ...
  const handleSubmit = async () => {
    ...
    // Auto-geocode from market or address
    let lat = form.latitude || null
    let lng = form.longitude || null

    if (!lat || !lng) {
      setGeocoding(true)
      const searchQuery = form.address ? `${form.address}, ${form.market}` : `${form.name}, ${form.market}`
      const coords = await geocodeAddress(searchQuery)
      if (coords) {
        lat = coords.lat; lng = coords.lng
        setGeocodeStatus('success')
      } else {
        // Fall back to just market
        const marketCoords = await geocodeAddress(form.market)
        if (marketCoords) { lat = marketCoords.lat; lng = marketCoords.lng; setGeocodeStatus('approx') }
        else setGeocodeStatus('failed')
      }
      setGeocoding(false)
    }
    ...
    const payload = {
      ...
      notes: form.notes || null, address: form.address || null,
      ...
      <div className="modal">
        ...
        <div className="form-group">
          <label className="form-label">
            Address
            <span style={{ fontSize:10, color:'var(--g600)', marginLeft:6, fontWeight:400 }}>— used for precise map placement</span>
          </label>
        ...
        <div style={{ background:'var(--g50)', border:'1px solid var(--g100)', borderRadius:7, padding:'8px 12px', fontSize:11, color:'var(--g700)', marginBottom:14, display:'flex', alignItems:'center', gap:6 }}>
          <span>📍</span>
          <span>{form.latitude && form.longitude ? `Coordinates saved — will appear on map` : `Map coordinates will be auto-detected from address or market when you save`}</span>
        </div>
          ...
          <div style={{ overflowX:'auto' }}>
            <table className="data-table">
              <thead><tr>
                ...
                <th>Map</th><th>Status</th><th>Actions</th>
              </tr></thead>
              <tbody>
                {filtered.map(a => {
                  ...
                  return (
                    <tr key={a.id}>
                      ...
                      <td>
                        <span style={{ fontSize:12 }} title={a.latitude ? `${a.latitude.toFixed(3)}, ${a.longitude.toFixed(3)}` : 'Not geocoded'}>
                          {a.latitude ? '📍' : '⬜'}
                        </span>
                      </td>
